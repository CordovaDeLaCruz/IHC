<div class="container">
    <div class="contenedor_cursos_detalle col-xs-12">
        <div class="contenedor_opciones col-xs-5">
            <div>
                <ul class="list-group">
                    <li class="list-group-item">Mis anuncios</li>
                    <li class="list-group-item">Bibliografía del Curso</li>
                    <li class="list-group-item">Lecturas</li>
                    <li class="list-group-item">Mis calificaciones</li>
                </ul>
            </div>
            <div>
                <ul class="list-group">
                    <li class="list-group-item">Semana 1</li>
                    <li class="list-group-item">
                    <a href="tarea.php">Semana 2</a>
                    </li>
                    <li class="list-group-item">Semana 3</li>
                    <li class="list-group-item">Semana 4</li>
                    <li class="list-group-item">Semana 5</li>
                    <li class="list-group-item">Semana 6</li>
                    <li class="list-group-item">Semana 7</li>
                    <li class="list-group-item">Semana 8</li>
                    <li class="list-group-item">Semana 9</li>
                    <li class="list-group-item">Semana 10</li>
                    <li class="list-group-item">Semana 11</li>
                    <li class="list-group-item">Semana 12</li>
                </ul>
            </div>
        </div>
        <div class="contenedor_tarea col-xs-7">
            <div class="tarea_titulo">
                <h1>
                    SEMANA 2
                </h1>
            </div>
            <div class="tarea_info">
                <h2>
                    SESION 2- PRESENCIAL
                </h2>
                <hr>
                <div class="tarea_info_lista">
                    <div>
                        <i class="material-icons md-36">book</i>
                    </div>

                    <h2>Logro de aprendizaje de la sesion:</h2>
                        
                </div>
                <div class="tarea_info_contenido">
                    <p>Al termino de esta sesion:</p>
                    <div class="tarea_contenido_lista">
                        <div>
                            <i class="material-icons md-34">done</i>
                        </div>  
                        <p>Logro 1</p>
                    </div>
                    <div class="tarea_contenido_lista">
                        <div>
                            <i class="material-icons md-34">done</i>
                        </div>  
                        <p>Logro 2</p>
                    </div>
                </div>
                <div class="tarea_info_lista">
                    <div>
                        <i class="material-icons md-36">assignment</i>
                    </div>

                    <h2>Temas</h2>
                </div>
                <div class="tarea_info_contenido">
                    <div class="tarea_contenido_lista">
                        <div>
                            <i class="material-icons md-34">book</i>
                        </div>  
                        <p>Diseño Centrado en Usuario</p>
                    </div>
                </div>
                <div class="tarea_info_lista">
                    <div>
                    <i class="material-icons md-36">assignment_late</i>
                    </div>
                    <a href="tarea-detalle.php">
                        <h2>Tareas pendientes</h2>
                    </a>
                </div>
                <div class="tarea_pendiente">

                </div>
            </div>
        </div>
    </div>
</div>